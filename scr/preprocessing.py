...
def extract_features(df):
    ...
    df['nameDestType'] = df['nameDest'].map(lambda x: 1 if x[0] == 'C' else 0)
    df['nameDestCount'] = df['nameDest'].map(df['nameDest'].value_counts())
    df['nameOrigCount'] = df['nameOrig'].map(df['nameOrig'].value_counts())
